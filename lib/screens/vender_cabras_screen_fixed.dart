...
import '../main.dart';
import '../models/cabra_oferta.dart';
import '../utils/image_utils.dart';
import '../utils/price_input_formatter.dart';
import '../widgets/bi_text.dart';

class VenderCabrasScreen extends StatefulWidget {
  const VenderCabrasScreen({super.key});

  @override
  State<VenderCabrasScreen> createState() => _VenderCabrasScreenState();
}

class _VenderCabrasScreenState extends State<VenderCabrasScreen> {
  ...
  Widget _campoFormaPago(BuildContext context) {
    return SizedBox(
      width: 640,
  ...
  Widget _campoDropdown(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: DropdownButtonFormField<String>(
        decoration: InputDecoration(
          labelText: bilingual ? AppLocalizations.bilingual('priceType') : loc.t('priceType'),
          filled: true,
          fillColor: const Color(0xFFE3F2FD),
          contentPadding: const EdgeInsets.symmetric(vertical: 10, horizontal: 16),
          isDense: true,
  ...
  Widget _campoPrecio(BuildContext context) {
    ...
    if (_tipoPrecioSeleccionado == 'eur_unidad') {
      validador = (value) {
        if (value == null || value.isEmpty) {
          return bilingual
              ? AppLocalizations.bilingual('errorPriceRequired')
    ...
    return SizedBox(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          labelText: bilingual ? AppLocalizations.bilingual('price') : loc.t('price'),
          filled: true,
          fillColor: const Color(0xFFE3F2FD),
          contentPadding: const EdgeInsets.symmetric(vertical: 10, horizontal: 16),
          isDense: true,
  ...
  Widget _campoFotos(BuildContext context) {
    final loc = AppLocalizations.of(context);
    final bilingual = MyApp.of(context)?.bilingualMode ?? false;
    return SizedBox(
      width: 640,
          ...
          OutlinedButton.icon(
            onPressed: () async {
              ...
              for (final f in toAdd) {
                try {
                  final key = _fotoCacheKey(f);
                  final raw = _fotoBytesCache[key] ?? await f.readAsBytes();
                  _fotoBytesCache[_fotoCacheKey(f)] = raw;
                  final compliance = await analyzeImageForCompliance(raw);
                  if (compliance.allowed) {
                    accepted.add(f);
                  } else {
                    rejected++;
                    rejectionReasons.addAll(compliance.reasons);
                  }
                } catch (_) {
                  rejected++;
                }
              ...
              setState(() {
                _fotos.addAll(accepted);
                _fotoBytesCache.removeWhere((key, _) {
                  return !_fotos.any((f) => _fotoCacheKey(f) == key);
                });
              });
              final int skippedForLimit = (newUnique.length - toAdd.length).clamp(0, 1000);
              final int addedCount = accepted.length;
              final List<String> messages = [];
              if (skippedForLimit > 0) {
                messages.add(loc.tf('photosAddedCountWithMax', {
                  'count': addedCount.toString(),
                  'max': max.toString(),
                }));
              } else {
                messages.add(loc.tf('photosAddedCount', {
                  'count': addedCount.toString(),
                }));
              }
              if (rejected > 0) {
                messages.add(loc.tf('photosRejectedCount', {'count': rejected.toString()}));
                if (rejectionReasons.isNotEmpty) {
                  final uniqueReasons = rejectionReasons.toSet().toList();
                  messages.add('Motivos: ${uniqueReasons.take(5).join(', ')}');
                }
              }
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                      content: Text(bilingual
                          ? AppLocalizations.bilingual('videoSelectedOk')
                          : loc.t('videoSelectedOk'))),
                );
              }
            },
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Form(
                        ...
                        child: Column(
                          children: [
                            _campoDropdown(context),
                            const SizedBox(height: 4),
                            _campoPrecio(context),
                            const SizedBox(height: 4),
                            _campoTexto(
                              bilingual
                                  ? AppLocalizations.bilingual('quantity')
                                  : loc.t('quantity'),
                            ...
                            _campoTexto(
                              bilingual
                                  ? AppLocalizations.bilingual('weightAverage')
                                  : loc.t('weightAverage'),
                              TextInputType.number,
                            ...
                            const SizedBox(height: 4),
                            _campoTexto(
                              bilingual ? AppLocalizations.bilingual('race') : loc.t('race'),
                              TextInputType.text,
                              maxLength: 10,
                              validator: (value) {
                                if (value == null || value.isEmpty) return null;
                            ...
                            _campoTexto(
                              ...
                              validator: (value) {
                                if (value == null || value.isEmpty) {
                                  return bilingual
                                      ? AppLocalizations.bilingual('errorAgeRequired')
                            ...
                            _campoTexto(
                              bilingual
                                  ? AppLocalizations.bilingual('postalCodeLabel')
                                  : loc.t('postalCodeLabel'),
                              TextInputType.number,
                              maxLength: 5,
                              validator: (value) {
                                if (value == null || value.isEmpty) return null;
                            ...
                            SizedBox(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Text(
                                      bilingual
                                          ? AppLocalizations.bilingual('defectQuestion')
                                          : loc.t('defectQuestion'),
                                    ...
                                    children: [
                                      ElevatedButton(
                                        ...
                                        child: Text(bilingual
                                            ? AppLocalizations.bilingual('yes')
                                            : loc.t('yes')),
                                      ),
                                  ...
                                  if (_diferenciaRespuesta == 'si') ...[
                                    ...
                                    TextFormField(
                                      ...
                                      decoration: InputDecoration(
                                        labelText: bilingual
                                            ? AppLocalizations.bilingual('defectDescribeLabel')
                                            : loc.t('defectDescribeLabel'),
                                        filled: true,
                                        fillColor: const Color(0xFFE3F2FD),
                                      ...
                                      validator: (value) {
                                        if (value == null || value.isEmpty) {
                                          return bilingual
                                              ? AppLocalizations.bilingual(
                                                  'errorDefectDescriptionRequired')
                                              : loc.t('errorDefectDescriptionRequired');
                                        }
                                        final exp = RegExp(
                                            r'^[a-zA-ZÃ¡Ã©Ã­Ã³ÃºÃÃ‰ÃÃ“ÃšÃ¼ÃœÃ±Ã‘ ]{1,40}$');
                                        if (!exp.hasMatch(value)) {
                                          return bilingual
                                              ? AppLocalizations.bilingual('errorOnlyLettersMax')
                                              : loc.t('errorOnlyLettersMax');
                                        }
                                        return null;
                                      },
                                    ),
                                    ...
                                    ElevatedButton(
                                      ...
                                      child: _isSavingDeclaracion
                                          ...
                                          : _declaracionGuardada
                                              ...
                                              : Text(bilingual
                                                  ? AppLocalizations.bilingual('accept')
                                                  : loc.t('accept')),
                                    ),
                                  ],
                                  if (_diferenciaRespuesta == 'no') ...[
                                    ...
                                    Text(
                                      bilingual
                                          ? AppLocalizations.bilingual('declarationNoDefect')
                                          : loc.t('declarationNoDefect'),
                                      style: const TextStyle(fontSize: 14),
                                    ),
                                    const SizedBox(height: 8),
                                    ElevatedButton(
                            ...
                            ElevatedButton(
                              onPressed: _isSavingOferta
                                  ...
                                  : () async {
                                      ...
                                      String add(String key) {
                                        return "${bilingual ? AppLocalizations.bilingual(key) : loc.t(key)}\n";
                                      }
                                      ...
                                      if (_fotos.length < 2) errorMsg += add('errorAtLeast2Photos');
                                      if (_fotos.length > 10) errorMsg += add('errorMax10Photos');
                                      if (!_declaracionGuardada) {
                                        errorMsg += add('errorDeclarationAcceptanceRequired');
                                      }
                                      ...
                                      try {
                                          ...
                                          for (int i = start; i < end; i++) {
                                            futures.add(() async {
                                              ...
                                              final ref = FirebaseStorage.instance.ref().child(
                                                  'users/${user.uid}/offers/cabras/$offerId/photos/$i.jpg');
                                              try {
                                        ...
                                        if (_video != null) {
                                          try {
                                            final refVideo = FirebaseStorage.instance.ref().child(
                                                'users/${user.uid}/offers/cabras/$offerId/video.mp4');
                                            final vbytes =
                                        ...
                                        // Notify success (simple message, no ID)
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(
                                            content: Text(
                                              bilingual
                                                  ? AppLocalizations.bilingual(
                                                      'offerPublishedSuccessSimple')
                                                  : loc.t('offerPublishedSuccessSimple'),
                                            ),
                                          ),
                                        );
                                        setState(() {
                                          ...
                                          SnackBar(
                                            content: Text(
                                              bilingual
                                                  ? AppLocalizations.bilingual(
                                                      'errorUploadingOffer')
                                                  : loc.tf('errorUploadingOffer', {'error': '$e'}),
                                            ),
                                          ),
                                        );
                                      }
                              ...
                              child: _isSavingOferta
                                          ...
                                          Padding(
                                            ...
                                            child: Text(
                                              (bilingual
                                                  ...
                                                  : loc.tf('uploadingPhotosProgress', {
                                                      'percent':
                                  ...
                                  : Text(bilingual
                                      ? AppLocalizations.bilingual('publishOffer')
                                      : loc.t('publishOffer')),
                            ),
                          ],
                        ),
