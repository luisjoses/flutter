...
import '../models/cerdo_oferta.dart';
import '../utils/image_utils.dart';
import '../utils/price_input_formatter.dart';

class VenderCerdosScreen extends StatefulWidget {
  const VenderCerdosScreen({super.key});

  @override
  State<VenderCerdosScreen> createState() => _VenderCerdosScreenState();
}

class _VenderCerdosScreenState extends State<VenderCerdosScreen> {
  ...
  Widget _campoTexto(String label, TextInputType tipo,
    ...
    return SizedBox(
      ...
      child: TextFormField(
        ...
        inputFormatters: [
          if (label == 'Cantidad') FilteringTextInputFormatter.allow(RegExp(r'^[0-9]{0,3}')),
          if (label == 'Peso medio (por animal)')
            FilteringTextInputFormatter.allow(RegExp(r'^[0-9]{0,4}')),
          if (label == 'Raza')
            FilteringTextInputFormatter.allow(
                RegExp(r'^[a-zA-ZÃ¡Ã©Ã­Ã³ÃºÃÃ‰ÃÃ“ÃšÃ¼ÃœÃ±Ã‘]{0,10}')),
          if (label == 'Edad (meses)') FilteringTextInputFormatter.allow(RegExp(r'^[0-9]{0,2}')),
          if (label == 'CÃ³digo Postal') FilteringTextInputFormatter.allow(RegExp(r'^[0-9]{0,5}')),
        ],
      ),
    );
  }
  ...
  Widget _campoPrecio() {
    ...
    if (_tipoPrecioSeleccionado == 'eur_unidad') {
      ...
      maxLength = 5; // separator shouldn't count towards digits
      inputFormatters = [
          ...
          OutlinedButton.icon(
            onPressed: () async {
              ...
              if (pickedFiles.isNotEmpty) {
                const max = 10;
                final selected = pickedFiles.take(max).toList();
                final Map<String, Uint8List> newCache = <String, Uint8List>{};
                final List<XFile> cacheable = <XFile>[];
                for (final file in selected) {
                  try {
                    final key = _fotoCacheKey(file);
                    newCache[key] = _fotoBytesCache[key] ?? await file.readAsBytes();
                    cacheable.add(file);
                  } catch (_) {
                ...
                if (pickedFiles.length > max) {
                  ...
                  final loc = AppLocalizations.of(context);
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(loc.t('photosSelectedOk'))),
                  );
                }
              }
            },
                                          ...
                                          for (int i = start; i < end; i++) {
                                            futures.add(() async {
                                              ...
                                              final ref = FirebaseStorage.instance.ref().child(
                                                  'users/${user.uid}/offers/cerdos/$offerId/photos/$i.jpg');
                                              try {
                                        ...
                                        if (_video != null) {
                                          try {
                                            final refVideo = FirebaseStorage.instance.ref().child(
                                                'users/${user.uid}/offers/cerdos/$offerId/video.mp4');
                                            final vbytes =
